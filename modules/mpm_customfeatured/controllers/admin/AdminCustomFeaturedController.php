...
class AdminCustomFeaturedController extends ModuleAdminController
{
  private $_idShop;
  private $_idLang;
  private $_homeFeatured;
  protected $position_identifier = 'id_customfeatured';

  public function __construct()
  {
    $this->className = 'customFeatured';
    $this->table = 'customfeatured';
    $this->bootstrap = true;
    $this->lang = true;
    $this->edit = true;
    $this->delete = true;
    ...
    $this->position_identifier = 'id_customfeatured';
    ...
    $this->fields_list = array(
      'id_customfeatured' => array(
        'title' => $this->l('ID'),
        'search' => true,
        'onclick' => false,
        'filter_key' => 'a!id_customfeatured',
        'width' => 20
      ),
      'title' => array(
        'title' => $this->l('Title'),
  ...
  {
    parent::init();
    if (Shop::getContext() == Shop::CONTEXT_SHOP && Shop::isFeatureActive() && Tools::getValue('viewcustomfeatured') === false)
  ...
  public function renderForm(){
    ...
    $this->fields_value['token_featured'] = Tools::getAdminTokenLite('AdminCustomFeatured');
  ...
  public function getProductBlock($id_lang, $id_shop, $ids){
    ...
    $data = Context::getContext()->smarty->createTemplate(_PS_MODULE_DIR_ . 'mpm_customfeatured/views/templates/hook/productBlock.tpl');
  ...
  public function getProductList($id_lang, $id_shop, $ids)
  {
    ...
    $data = Context::getContext()->smarty->createTemplate(_PS_MODULE_DIR_ . 'mpm_customfeatured/views/templates/hook/productList.tpl');
    $data->assign(
      array(
        'id_shop' => $id_shop,
        'id_lang' => $id_lang,
  ...
  {
    $homefeatured = Tools::getValue('customfeatured');
    foreach($homefeatured as $key => $value){
      $value = explode('_', $value);
      Db::getInstance()->update('customfeatured', array('position' => (int)$key), 'id_customfeatured='.(int)$value[2]);
    }
  }
