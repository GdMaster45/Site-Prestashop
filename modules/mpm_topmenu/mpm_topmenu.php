...
class mpm_topmenu extends Module
{
  ...
  private function _setDataDb(){
    ...
    $data = array(
      array(
        'title'           => 'Blog',
        'link'            => Tools::getShopDomainSsl(true) . __PS_BASE_URI__ . 'blog/',
        'open_new_window' => 0,
      ),
      array(
        'title'           => 'Contact us',
  ...
  public function hookDisplayHeader()
  {
      ...
      $this->context->controller->registerStylesheet('mpm_homefeatured', 'modules/'.$this->name.'/views/css/mpm_topmenu.css',  array('media' => 'all', 'priority' => 900));
      $this->context->controller->registerJavascript('mpm_homefeatured', 'modules/'.$this->name.'/views/js/mpm_topmenu.js',  array('position' => 'bottom', 'priority' => 150));
  ...
  public function hookDisplayTopMenu()
  {
    ...
    foreach ($menu_items as $key => $value){
      $menu_items[$key]['columns'] = $this->getContentColumns($this->_idLang, $this->_idShop, $value['id_topmenu']);
  ...
  public function getSuppliersBlock($id_lang, $id_shop, $value){
    ...
    foreach ($ids as $key => $val){
      ...
      $links[$key]['link'] = Context::getContext()->link->getSupplierLink((int)$val, null, (int)$id_lang, $id_shop);
    }
  ...
  public function getBrandsBlock($id_lang, $id_shop, $value){
    ...
    foreach ($ids as $key => $val){
      ...
      $links[$key]['link'] = Context::getContext()->link->getManufacturerLink((int)$value, null, null, (int)$id_lang, $id_shop);
    }
